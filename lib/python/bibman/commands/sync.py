...
class SyncCommand:
    ...
    def walk_path(self):
        if not os.path.isdir(self.args.path):
            logging.error("Could not find directory: {}".format(self.args.path))
            return
...
def register_args(parser):
    parser.add_argument("-p", "--path", metavar="PATH", type=str,
            ...
            help="Path to scan and synchronise BIBFILE with.")
    parser.add_argument("--extlist", type=str,
            dest="extlist", default="pdf",
            help="Command separated list of file-extensions to consider for sync. [Default:pdf]")
    ...
    parser.add_argument("-e", "--exclude", metavar="EXCLUDE", type=str,
            ...
            help="Comma separated list of BibTeX files to exclude new entries from.")
